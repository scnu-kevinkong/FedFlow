"""
Modified from https://github.com/hendrycks/robustness/blob/master/ImageNet-C/create_c/make_cifar_c.py
"""
...
def frost(x, severity=1):
    ...
    filename = ['assets/frost1.png', 'assets/frost2.png', 
                'assets/frost3.png', 'assets/frost4.jpg', 
                'assets/frost5.jpg', 'assets/frost6.jpg'][idx]
...
"""
select 10 augmentations, each with 5 levels
(to be distributed to each of the first 50 clients)
1) motion blur
2) defocus blur
3) gaussian noise
...
if __name__ == "__main__":
    ...
    print("Generating CIFAR10-C")
    DATA_PATH = Path(os.environ["DATA_PATH"])
    CIFAR10_PATH = DATA_PATH / "cifar10"  
    CIFAR10C_PATH = DATA_PATH / "cifar10c" 

    if not os.path.exists(CIFAR10C_PATH):
        os.mkdir(CIFAR10C_PATH)

    train_data = dset.CIFAR10(root=CIFAR10_PATH, train=True, download=True)
    test_data = dset.CIFAR10(root=CIFAR10_PATH, train=False, download=True)
    full_set = ConcatDataset([train_data, test_data])

    convert_img = trn.Compose([trn.ToTensor(), trn.ToPILImage()])

    for method_name in d.keys():
        for severity in range(1, 6):
            print(f'Creating images for the corruption {method_name} with severity {severity}')
            cifar_c, labels = [], []
            corruption = lambda clean_img: d[method_name](clean_img, severity)
            for img, label in tqdm(zip(np.concatenate([s.data for s in full_set.datasets]), np.concatenate([s.targets for s in full_set.datasets]))):
                labels.append(label)
                cifar_c.append(np.uint8(corruption(convert_img(img))))

            np.save(CIFAR10C_PATH / f"{d[method_name].__name__}_{severity}.npy",
                    np.array(cifar_c).astype(np.uint8))

            np.save(CIFAR10C_PATH / f"{d[method_name].__name__}_{severity}_labels.npy",
                    np.array(labels).astype(np.uint8))

    print("Generating CIFAR100-C")
    DATA_PATH = Path(os.environ["DATA_PATH"])
    CIFAR100_PATH = DATA_PATH / "cifar100"  
    CIFAR100C_PATH = DATA_PATH / "cifar100c" 

    if not os.path.exists(CIFAR100C_PATH):
        os.mkdir(CIFAR100C_PATH)

    train_data = dset.CIFAR100(root=CIFAR100_PATH, train=True, download=True)
    test_data = dset.CIFAR100(root=CIFAR100_PATH, train=False, download=True)
    full_set = ConcatDataset([train_data, test_data])
    ...
    for method_name in d.keys():
        for severity in range(1, 6):
            ...
            np.save(CIFAR100C_PATH / f"{d[method_name].__name__}_{severity}.npy",
                    np.array(cifar_c).astype(np.uint8))

            np.save(CIFAR100C_PATH / f"{d[method_name].__name__}_{severity}_labels.npy",
                    np.array(labels).astype(np.uint8))
